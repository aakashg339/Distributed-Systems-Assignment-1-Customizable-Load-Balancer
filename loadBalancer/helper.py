...
def get_container_iD(container_name):
    return os.popen(f' docker ps -aqf "name={container_name}"').read().strip()

def createServer(id, container_name, port):
    # os.popen(f" docker stop {container_name}")
    # os.popen(f" docker rm {container_name}")
    return os.popen(f' docker run --name {container_name} --network my_network -e SERVER_ID={id} -p {port}:5000 -d serverimage').read()
